#!/usr/bin/env python3
"""
aethersense-hap-bridge.py — ADR-125 §2.1.c production bridge (Tier 1+2 iter 3).

One HAP bridge `AetherSense Sensing` carrying N child accessories — one per
...
State per room comes from `/tmp/aethersense-state.<room>.json`. A C6
provisioned with `--room kitchen` writes `/tmp/aethersense-state.kitchen.json`;
the bridge picks it up automatically on next launch.

For backwards-compat with iter 1-2 (one-room setup) the legacy
`/tmp/aethersense-state.json` still feeds the room named via `--legacy-room`
(default: `Living Room`).

This script intentionally uses port 51827 (one above the test bridge's
51826) and a separate persist file so the iter-1-paired `AetherSense Test
...
Usage:
    python3 aethersense-hap-bridge.py                  # auto-discover rooms
    python3 aethersense-hap-bridge.py --rooms "Living Room,Bedroom,Office"
"""
...
# extension to the standard MotionSensor service. The UUID is AetherSense-specific
...
STATE_DIR = Path(os.path.expanduser("~/.aethersense-hap-prod"))
STATE_DIR.mkdir(exist_ok=True)
PERSIST_FILE = STATE_DIR / "bridge.state"
SETUP_CODE_FILE = STATE_DIR / "setup-code.txt"

LEGACY_STATE = Path("/tmp/aethersense-state.json")
ROOM_STATE_GLOB = re.compile(r"^/tmp/aethersense-state\.([^/]+)\.json$")


def discover_rooms_from_filesystem() -> list[tuple[str, Path]]:
    """Scan /tmp for aethersense-state.<room>.json files and return (room, path)."""
    rooms: list[tuple[str, Path]] = []
    for entry in Path("/tmp").glob("aethersense-state.*.json"):
...
def main() -> int:
    ...
    if args.rooms:
        for r in [s.strip() for s in args.rooms.split(",") if s.strip()]:
            ...
            rooms.append((r, Path(f"/tmp/aethersense-state.{slug}.json")))
    else:
    ...
    if not rooms:
        sys.stderr.write(
            ...
            "c6-presence-watcher.py first (writes /tmp/aethersense-state.json), "
    ...
    print(f"[bridge] HAP bridge advertising as 'AetherSense Sensing' (production)",
          flush=True)
