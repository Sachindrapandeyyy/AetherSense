"""
Phase 3 — AetherSenseOccDataset: WorldGraph history → OccWorld-format tensors.
...
Class mapping (18-class OccWorld schema):
    AetherSense class    → OccWorld index   nuScenes label
...
Usage (standalone validation):
    python3 scripts/aethersense_occ_dataset.py --snapshots /tmp/snapshots/ --check

Usage (as OccWorld dataset replacement):
    from aethersense_occ_dataset import AetherSenseOccDataset
    ds = AetherSenseOccDataset(snapshot_dir="/tmp/snapshots", return_len=16)
...
def _cli_check() -> None:
    """Validate AetherSenseOccDataset with synthetic data."""
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ds = AetherSenseOccDataset(tmpdir, return_len=16)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AetherSenseOccDataset — Phase 3 domain adapter")
    ...
    if args.check:
        ...
        ds = AetherSenseOccDataset(args.snapshots)
