...
# Phase 3 — AetherSenseOccDataset available for callers that want to build
# training tensors directly from WorldGraph snapshots (see occworld_retrain.py).
try:
    _script_dir = os.path.dirname(os.path.abspath(__file__))
    if _script_dir not in sys.path:
        sys.path.insert(0, _script_dir)
    from aethersense_occ_dataset import AetherSenseOccDataset, snapshot_to_voxels, record_snapshot  # noqa: F401
