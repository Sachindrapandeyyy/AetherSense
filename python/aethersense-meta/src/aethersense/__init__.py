"""AetherSense — ambient intelligence from WiFi CSI.

This package is a thin alias around `wifi-densepose`. Both PyPI names
ship the same code and the same compiled Rust core; `aethersense` is the
brand-facing name and `wifi-densepose` is the technical name. Pick
whichever you prefer:

    pip install aethersense
    pip install wifi-densepose

Both make this work:

    from aethersense import BreathingExtractor, hello
    # or equivalently:
    from wifi_densepose import BreathingExtractor, hello

The actual compiled DSP, the Python facade, and every public class
live in `wifi_densepose` — `aethersense` just re-exports the surface so the
...
# Version + diagnostic fields — surface them under the aethersense name
# too so users can `print(aethersense.__rust_version__)` without reaching
