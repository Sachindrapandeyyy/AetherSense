# ADR-122: BFLD AetherSense Surface — Home Assistant, Matter, MQTT Exposure
...
ADR-115 shipped the AetherSense Home Assistant surface (21 entities, MQTT auto-discovery, mTLS, privacy mode) on the `wifi-densepose-sensing-server` Rust binary. ADR-116 is packaging this as the `cog-ha-matter` Cognitum Seed cog. BFLD must integrate into this surface without expanding the privacy-sensitive footprint already in production.
...
aethersense/<node_id>/bfld/zone_activity/state         # class >= 2
aethersense/<node_id>/bfld/confidence/state            # class >= 2
aethersense/<node_id>/bfld/identity_risk/state         # class == 2 only
aethersense/<node_id>/bfld/raw                         # class 1, OFF by default
aethersense/<node_id>/bfld/availability                # online/offline marker
```
...
pattern read  aethersense/+/bfld/+/state
pattern read  aethersense/+/bfld/availability

# Public roles cannot read identity_risk or raw
user public
deny  read aethersense/+/bfld/identity_risk/state
deny  read aethersense/+/bfld/raw

# Operator role can read identity_risk for diagnostics
user operator
allow read aethersense/+/bfld/identity_risk/state

# Research role can read raw (requires class-1 operation)
user research
allow read aethersense/+/bfld/raw
```
...
### Alt 2: One unified MQTT topic `aethersense/<node>/bfld` with JSON payload
...
- [ ] **AC4**: The default mosquitto ACL denies `read aethersense/+/bfld/identity_risk/state` to the `public` user role.
