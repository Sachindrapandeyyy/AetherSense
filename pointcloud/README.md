# AetherSense — Live 3D Point Cloud Viewer

Hosted at: https://ruvnet.github.io/AetherSense/pointcloud/

## Modes

- Default — synthetic in-browser demo (no backend, no network calls).
- `?backend=auto` — fetch from `/api/splats` on the same origin
  (only works when the viewer is served by `aethersense-pointcloud serve`).
- `?backend=<url>` — fetch from `<url>/api/splats`. The intended
  local-ESP32 use is `?backend=http://127.0.0.1:9880`: run
  `aethersense-pointcloud serve --bind 127.0.0.1:9880` on the same
