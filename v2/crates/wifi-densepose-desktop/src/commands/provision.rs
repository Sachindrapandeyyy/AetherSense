...
const PROVISION_MAGIC: &[u8] = b"AETHER_NVS";
...
pub async fn read_nvs(port: String) -> Result<ProvisioningConfig, String> {
    ...
    // Send read command
    tokio::io::AsyncWriteExt::write_all(&mut writer, b"AETHER_NVS_READ\n")
        .await
...
pub async fn erase_nvs(port: String) -> Result<ProvisionResult, String> {
    ...
    // Send erase command
    tokio::io::AsyncWriteExt::write_all(&mut writer, b"AETHER_NVS_ERASE\n")
        .await
...
mod tests {
    ...
    fn test_provision_header() {
        ...
        assert_eq!(&bytes[0..10], b"AETHER_NVS");
    }
}
