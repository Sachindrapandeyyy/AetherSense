//! AetherSense sensing primitives → HAP characteristic mapping (ADR-125 §2.1.d).
//!
//! Per ADR-125, AetherSense's privacy-class-2/3 events map to HomeKit primitives
//! as semantic ambient signals, not surveillance events:
//!
//! | AetherSense primitive | HAP service | Rationale |
//! |-----------------|-------------|-----------|
...
/// Parsed AetherSense edge vitals event from the sensing-server.
///
...
/// Maps `EdgeVitals` to a `Vec<AccessoryMapping>` — one per AetherSense primitive
/// that should be exposed as a distinct HAP service (child accessory).
pub struct AetherSenseToHapMapper;

impl AetherSenseToHapMapper {
...
mod tests {
    ...
    fn presence_true_maps_to_occupancy_detected_1() {
        ...
        let mappings = AetherSenseToHapMapper::map(&vitals);
