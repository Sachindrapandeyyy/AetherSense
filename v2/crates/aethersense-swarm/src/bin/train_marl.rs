//! MARL training entry point for aethersense-swarm (ADR-148 M4).
//!
...
use aethersense_swarm::config::SwarmConfig;
use aethersense_swarm::integration::telemetry::{DroneFrame, TelemetryRecorder};
use aethersense_swarm::marl::candle_ppo::{CandlePpoConfig, CandleTrainer};
use aethersense_swarm::marl::learning::{shaped_reward, CuriosityModule, LearningPattern};
use aethersense_swarm::marl::observation::LocalObservation;
use aethersense_swarm::marl::reward::{RewardCalculator, RewardContext};
use aethersense_swarm::planning::patterns::{FlightPattern, PatternContext};
use aethersense_swarm::types::{DroneState, NodeId, Position3D, Velocity3D};
...
fn parse_args() -> Args {
    ...
    while i < argv.len() {
        ...
        match argv[i].as_str() {
            ...
            "-h" | "--help" => {
                println!(
                    "train_marl — aethersense-swarm MARL training (ADR-148 M4)\n\
                     \nOptions:\n  \
