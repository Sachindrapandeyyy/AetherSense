...
//!
//! ```text
//! cargo run -p aethersense-swarm --bin eval_swarm -- \
//!   --seeds 10 --episodes 10 --out crates/aethersense-swarm/evals/RESULTS.md
//! ```

use std::path::PathBuf;

use aethersense_swarm::evals::metrics::AggregateMetrics;
use aethersense_swarm::evals::report::render_results_md;
use aethersense_swarm::evals::runner::{run_matrix, EvalConfig};
use aethersense_swarm::planning::patterns::FlightPattern;

fn main() {
    let args: Vec<String> = std::env::args().collect();
    let mut seeds = 10usize;
    let mut episodes = 10usize;
    let mut out = PathBuf::from("crates/aethersense-swarm/evals/RESULTS.md");
            ...
            "--help" | "-h" => {
                eprintln!(
                    "eval_swarm — ADR-149 Stage-1 kinematic evaluator\n\
                     ...
                     Defaults: --seeds 10 --episodes 10 --out crates/aethersense-swarm/evals/RESULTS.md"
                );
                return;
            }
            other => {
