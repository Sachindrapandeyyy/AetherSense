//! AetherSense three-metric acceptance test (ADR-031).
//!
...
impl std::fmt::Display for AetherSenseTier {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AetherSenseTier::Fail => write!(f, "FAIL"),
            AetherSenseTier::Bronze => write!(f, "BRONZE"),
            AetherSenseTier::Silver => write!(f, "SILVER"),
            AetherSenseTier::Gold => write!(f, "GOLD"),
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct AetherSenseAcceptanceResult {
    ...
    pub tier: AetherSenseTier,
}

impl AetherSenseAcceptanceResult {
    /// A human-readable summary of the acceptance test.
    pub fn summary(&self) -> String {
        format!(
            "AetherSense Tier={} | PCK={:.3} OKS={:.3} | MOTA={:.3} IDsw={} | Breathing={:.1}BPM err",
            self.tier,
...
pub fn determine_tier(
    ...
    if !tracking.passes {
        return AetherSenseTier::Fail;
    }
    // Bronze: only tracking passes.
    if !joint_error.passes {
        return AetherSenseTier::Bronze;
    }
    // Silver: tracking + joint error pass.
    if !vital_signs.passes {
        return AetherSenseTier::Silver;
    }
    // Gold: all pass.
    AetherSenseTier::Gold
}
...
mod tests {
    ...
    fn tier_determination_gold() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), AetherSenseTier::Gold);
    }
    ...
    fn tier_determination_silver() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), AetherSenseTier::Silver);
    }
    ...
    fn tier_determination_bronze() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), AetherSenseTier::Bronze);
    }
    ...
    fn tier_determination_fail() {
        ...
        assert_eq!(determine_tier(&je, &tr, &vs), AetherSenseTier::Fail);
    }

    #[test]
    fn tier_ordering() {
        assert!(AetherSenseTier::Gold > AetherSenseTier::Silver);
        assert!(AetherSenseTier::Silver > AetherSenseTier::Bronze);
        assert!(AetherSenseTier::Bronze > AetherSenseTier::Fail);
    }
