...
Home Assistant (HA) is the dominant open-source home automation hub with more than 500,000 active installs (ADR-115 §1.2 competitive scan). Every prior AetherSense integration decision has been made with HA as a given constraint: ADR-115 built an MQTT auto-discovery publisher to fit inside HA, ADR-116 packaged it as a Cognitum Seed cog, ADR-122 extended it with BFLD presence events, and ADR-125 layered a native HAP bridge on top of the same stack.

This approach yields functioning integrations, but it positions AetherSense permanently as a **guest in someone else's hub**. The architectural limits of Python HA are not just cosmetic:

| Limit | Impact on AetherSense's roadmap |
|---|---|
...
1. **ADR-117 (PIP-PHOENIX)** — `wifi-densepose==2.0.0a1` + `aethersense==2.0.0a1` on PyPI as PyO3/maturin wheels, providing a Python developer surface over the Rust sensing core.
2. **ADR-118 (BFLD)** — a complete beamforming feedback capture and privacy-risk scoring layer, proving that AetherSense's sensing stack can be a compliance instrument, not just a sensor.
...
The gap that remains: there is no hub that treats all of these as **native first-class features** rather than bolt-on integrations. HOMECORE fills that gap by porting the HA data model and API surface to Rust, replacing HA's Python internals with the AetherSense Rust crates, and wrapping community integrations in WASM sandboxes.
...
Build **HOMECORE**: a native Rust + WASM + TypeScript implementation of the Home Assistant hub contract, integrated with the AetherSense sensing platform, the ruflo agent toolchain, and the ruvector vector layer.
...
│                                                               │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  AetherSense sensing core (wifi-densepose-sensing-server) │   │
...
### 5.1 Governance via AETHERSENSE-POLICY (ADR-124 §4.1a)

Every HOMECORE component that returns biometric data (presence, HR/BR, pose keypoints, BFLD identity-risk) MUST route through the AETHERSENSE-POLICY layer defined in ADR-124 §4.1a. The policy store is the same `~/.config/rvagent/policy.json` used by `@ruvnet/rvagent`. HOMECORE is a first-class policy principal — its agent ID in the policy store is `homecore`.
...
### Alt-A: Contribute AetherSense sensing features upstream to Python HA
...
Keep Python HA as the runtime; expose AetherSense sensing primitives via PyO3 bindings so they run at native speed inside the Python HA process.
...
Port AetherSense's sensing stack on top of an alternative hub (openHAB/Java, Domoticz/C++).
...
- `docs/adr/ADR-124-rvagent-mcp-ruvector-npm-integration.md` — SENSE-BRIDGE; AETHERSENSE-POLICY §4.1a; multi-modal normalization §11.3
- `docs/adr/ADR-125-aethersense-apple-home-native-hap-bridge.md` — APPLE-FABRIC HAP bridge
