# ADR-125: AetherSense ↔ Apple Home native HAP bridge — direct HomeKit accessory advertisement from the Seed
...
| **Date** | 2026-05-25 |
| **Deciders** | ruv |
| **Codename** | **APPLE-FABRIC** — AetherSense speaks HomeKit directly so Apple HomePod / Apple TV act as the discovery + automation surface with zero Home-Assistant middle layer |
| **Relates to** | [ADR-115](ADR-115-home-assistant-integration.md) (HA-DISCO MQTT publisher), [ADR-116](ADR-116-cog-ha-matter-seed.md) (cog-ha-matter §P7 left HAP/Matter as a feature-flag stub), [ADR-118](ADR-118-bfld-beamforming-feedback-layer-for-detection.md) (BFLD presence + identity-risk events), [ADR-122](ADR-122-bfld-aethersense-ha-matter-exposure.md) (BFLD HA/Matter exposure) |
| **Tracking issue** | TBD |
...
```text
AetherSense sensing-server ──► cog-ha-matter (MQTT HA-DISCO + HA-MIND)
                              ↓
...
1. **The BFLD privacy gate (ADR-118 / 120 / 121) is shipped.** Class-2 and class-3 frames are the only ones eligible to cross the Matter boundary (ADR-122 §2.4). Without that gate we could not safely expose AetherSense signals to a consumer ecosystem. With it, every Anonymous / Restricted event is safe to advertise as a HomeKit sensor.
2. **`@ruvnet/rvagent` (ADR-124) is on npm.** The MCP surface that lets agents query AetherSense is live. A first-class Apple-Home presence widens AetherSense's reach from "agents that speak MCP" to "anyone with an iPhone and a HomePod" — the consumer wedge.
...
| Layer | AetherSense contributes | Apple Home contributes |
|-------|---------------------|------------------------|
...
AetherSense supplies the **invisible cognition layer** Apple cannot provide on its own; Apple supplies the **distribution and UX** that an open sensing stack cannot bootstrap. Direct HAP integration removes the only structural barrier between those two layers — Home Assistant as a mandatory intermediary.
...
Add a tiny Python entrypoint `bridges/hap-python/aethersense_hap.py` using the well-maintained [`HAP-python`](https://github.com/ikalchev/HAP-python) library. The Dockerfile gets a thin Python runtime stage; the entrypoint script polls `sensing-server` over HTTP and pushes characteristic updates into the HAP loop.

```python
# bridges/hap-python/aethersense_hap.py (≈80 LOC)
...
class AetherSenseSensor(Accessory):
...
driver.add_accessory(accessory=AetherSenseSensor(driver, 'AetherSense Sense'))
driver.start()
```
...
2. Tap `AetherSense Sense` (appears via mDNS automatically)
3. Enter the setup code shown in `docker logs` (or pinned in env)
4. Done — Siri can say "Hey Siri, is anyone in the living room?"

Replace the `motion_present` / `occupancy` mappings progressively as AetherSense capabilities mature: BFLD class-2 `presence` event → `OccupancyDetected`; BFLD class-3 `identity_risk_score > threshold` → `SecuritySystemCurrentState`; `breathing_present` → `OccupancyDetected` (sleep room); `fall_risk` → a programmable switch that fires an Apple Home automation.

Acceptance criteria for 2.1.a:

- A1: `docker run ... hap-accessory --privacy-mode` advertises an `_hap._tcp` service that the HomePod sees within 30s (`dns-sd -B _hap._tcp local.` on a peer Mac shows `AetherSense Sense`).
...
- A4: Restarting the container preserves the pairing (HAP state persisted under `/var/lib/aethersense-hap/`).
- A5: Privacy: the entrypoint refuses to launch without `--privacy-mode` when `AETHERSENSE_BFLD_PRIVACY_CLASS` is unset, matching the structural invariant I1 (Raw BFI never exits the node — ADR-118 §2.2).
...
- **No direct connection to the HomePod.** As the framing in §1.1 makes explicit, AetherSense never opens a socket to the HomePod. It advertises; the HomePod discovers.
- **No iCloud account binding.** HAP pairing is local-network-only by design — AetherSense gets adoption without ever touching Apple ID, which is a privacy story we keep cleanly.
...
3. **P3** (when matter-rs stabilizes) — Matter Controller path (still AetherSense-as-accessory, but using the Matter clusters rather than HAP-1.1 services). The Cognitum Cog gains a Matter QR code; pairing flow widens to "any Matter-capable controller, not just Apple."
...
- **Direct discoverability on Apple Home.** A Seed in the kitchen appears as `AetherSense Sense` in the Home app within seconds of `docker run`. No HA, no MQTT broker, no Home-Assistant HomeKit Bridge add-on.
- **Siri natively answers AetherSense questions.** "Hey Siri, is anyone in the kitchen?" — the question reaches the HomeKit characteristic without any custom skill or HA template sensor.
- **Apple-Home automations gain ambient triggers** AetherSense already produces (presence, breathing, fall, identity-risk) for free — they become first-class automation triggers in the Home app's UI.
- **Strategically corrects AetherSense's distribution problem.** The Apple Home installed base is the largest consumer surface for HomeKit-grade accessories. AetherSense's sensing IP becomes addressable to that base without an SDK port.
...
- **Network-mode constraint.** HAP pairing needs the controller and accessory on the same L2 segment (mDNS broadcasts). Operators who run AetherSense in a container behind a NAT/bridge need `--network host` or a macvlan — same constraint HA's HomeKit Bridge has, but worth documenting.
- **Pairing state persistence.** HAP-python stores pairing data in a local file; that state must survive container restarts. Volume-mount `/var/lib/aethersense-hap/` to a persistent location.
...
- **Apple's evolving requirements.** HomeKit Accessory Certification is required to put a HAP logo on hardware, not to ship a software accessory that pairs locally. AetherSense's container deployment is squarely in the "uncertified developer accessory" lane, which Apple explicitly permits for local pairing. Worth restating in the operator README.
...
docker run -d --name hap --network host \
    -v /var/lib/aethersense-hap:/var/lib/aethersense-hap \
    -e AETHERSENSE_BFLD_PRIVACY_CLASS=2 \
    ruvnet/wifi-densepose:latest hap-accessory --privacy-mode

# 3. From a Mac on the same LAN: should see AetherSense Sense as HAP
dns-sd -B _hap._tcp local.   # expect: "AetherSense Sense" within 30 s

# 4. From iPhone Home app: Add Accessory → More Options → AetherSense Sense
...
- ADR-122 — BFLD AetherSense HA/Matter exposure (current MQTT-based bridge that this ADR's HAP-native path complements)
