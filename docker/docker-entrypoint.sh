...
# The pre-fix default was: empty AETHERSENSE_API_TOKEN (auth off) + --bind-addr
...
#   * Set AETHERSENSE_API_TOKEN to a strong secret → auth enabled on /api/v1/*.
#   * Set AETHERSENSE_ALLOW_UNAUTHENTICATED=1 → preserves the pre-fix behaviour;
#     only safe on an isolated trust boundary.
#   * Set AETHERSENSE_BIND_ADDR to a loopback / private interface → unauth is fine
...
case "${1:-}" in
    ...
    *)
        if [ -z "${AETHERSENSE_API_TOKEN:-}" ] && [ "${AETHERSENSE_ALLOW_UNAUTHENTICATED:-}" != "1" ]; then
            ...
            __bind_default="${AETHERSENSE_BIND_ADDR:-0.0.0.0}"
            case "$__bind_default" in
                ...
                *)
                    ...
                    echo "[entrypoint]        posture: AETHERSENSE_API_TOKEN is unset AND bind is" >&2
                    ...
                    echo "[entrypoint]          docker run -e AETHERSENSE_API_TOKEN=\$(openssl rand -hex 32) ..." >&2
                    echo "[entrypoint]          docker run -e AETHERSENSE_BIND_ADDR=127.0.0.1 ..." >&2
                    echo "[entrypoint]          docker run -e AETHERSENSE_ALLOW_UNAUTHENTICATED=1 ...   # only on trusted network" >&2
                    echo "[entrypoint]        See https://github.com/ruvnet/AetherSense/issues/864" >&2
                    exit 64
                    ;;
            esac
        fi
...
if [ "${1#-}" != "$1" ] || [ -z "$1" ]; then
    set -- /app/sensing-server \
        ...
        --http-port 3000 \
        --ws-port 3001 \
        --bind-addr "${AETHERSENSE_BIND_ADDR:-0.0.0.0}" \
        "$@"
fi
