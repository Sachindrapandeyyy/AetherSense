...
Build and flash AetherSense ESP32 firmware. Variant + port: `$ARGUMENTS` (default `8mb`, port `COM8`).
...
3. **Flash.** Same subprocess with `[python, idf_py, '-p', 'COM8', 'flash']`, or:
   ...
   ```
4. **Confirm.** Serial monitor via pyserial on `COM8` @ 115200 (NOT `idf.py monitor` — it hangs in a subprocess). Then `cd v2 && cargo run -p wifi-densepose-sensing-server` — frames should arrive. If not: re-run `/aethersense-provision`, match the AP channel, drop any `--filter-mac`.
