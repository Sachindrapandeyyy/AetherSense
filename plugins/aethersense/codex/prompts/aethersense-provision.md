# /aethersense-provision — provision an ESP32 sensing node

Write NVS config to a AetherSense ESP32 node. Args: `$ARGUMENTS` (expect `--port`, `--ssid`, `--password`, `--target-ip`, optional `--channel`, `--filter-mac`). Default port `COM8`.
