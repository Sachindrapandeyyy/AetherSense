# AGENTS.md — AetherSense (WiFi-DensePose)

Project rules for Codex (and any agent) working in the `ruvnet/AetherSense` / `wifi-densepose` repo. Mirrors the Claude Code `aethersense` plugin.
...
| `aethersense-flash` | Build + flash ESP32 firmware (8MB / 4MB) |
| `aethersense-provision` | Provision WiFi creds + sink IP + channel/MAC overrides |
...
| `aethersense-rvagent` | Explore rvAgent + RVF agentic flows wiring into AetherSense |
