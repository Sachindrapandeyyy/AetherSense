#!/usr/bin/env bash
# Structural smoke test for the `aethersense` Claude Code plugin.
# Run from anywhere: bash plugins/aethersense/scripts/smoke.sh
set -u

# Resolve plugin root (this file lives in <root>/scripts/smoke.sh).
# Plugin lives at <repo>/plugins/aethersense ; marketplace manifest is at <repo>/.claude-plugin/marketplace.json
...
echo "aethersense plugin smoke test"
echo "root: $ROOT"
echo "repo: $REPO"
echo

# 1. repo-root marketplace.json exists, lists the aethersense plugin, points source at ./plugins/aethersense
if [ -f "$MARKET" ] && has '"aethersense"' "$MARKET" && has '"\./plugins/aethersense"' "$MARKET"; then ok "repo-root .claude-plugin/marketplace.json lists 'aethersense' with source ./plugins/aethersense"; else bad "marketplace.json missing / wrong location / wrong source ($MARKET)"; fi
...
EXPECTED_SKILLS="aethersense-quickstart aethersense-hardware-setup aethersense-configure aethersense-applications aethersense-model-training aethersense-advanced-sensing aethersense-cli-api aethersense-mmwave aethersense-verify"
SKILLS_PRESENT=1
...
ADR="$ROOT/docs/adrs/0001-aethersense-plugin-contract.md"
