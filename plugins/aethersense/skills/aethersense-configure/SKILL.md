---
name: aethersense-configure
description: Configure AetherSense — ESP32 sdkconfig variants, NVS provisioning, WiFi channel / MAC filter overrides (ADR-060), edge intelligence modules (ADR-041), sensing-server flags, multi-node mesh, and Cognitum Seed integration. Use when adjusting how a deployed AetherSense system behaves without changing code.
allowed-tools: Bash Read Write Edit Glob Grep
---

# AetherSense Configuration

Everything you can tune in a AetherSense deployment, from a one-line provision flag to a full mesh + Cognitum Seed setup.
...
Switch: `cp firmware/esp32-csi-node/sdkconfig.defaults.<variant> firmware/esp32-csi-node/sdkconfig.defaults`, then rebuild (see `aethersense-hardware-setup`). **Never test in mock mode** — the Kconfig fall-threshold bug only showed up with real CSI.
...
- Cross-viewpoint fusion combines nodes — see `aethersense-advanced-sensing`.
