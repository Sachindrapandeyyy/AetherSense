---
name: aethersense-rvagent
description: Explore and prototype rvAgent + RVF integration for AetherSense agentic flows. Use when working on cross-cog coordination, operator-facing agents reading BFLD / pose / vitals events live, or persisting agent state alongside sensing data in the same RVF container.
---

# AetherSense rvAgent + RVF integration

Surface area for wiring `vendor/ruvector/crates/rvAgent/` into AetherSense so the existing sensing pipeline becomes the substrate an agentic flow can read, reason about, and respond to.
...
Override the sensing-server URL via the `RVAGENT_SENSING_URL` env var (default `http://localhost:3000`). Source lives at `tools/aethersense-mcp/`; ADR-124 captures the design.
...
1. **RVF wire**: two new segment types (`SEG_AGENT_STATE = 0x08`, `SEG_DECISION = 0x09`) let rvAgent sessions interleave with AetherSense sensing sessions in the same blob.
...
ADR-124 (proposed) — "rvAgent + RVF integration for AetherSense agentic flows" — would capture segment assignments, cog-subagent contract, and the privacy-class composition rule. Land before scaffolding `v2/crates/wifi-densepose-agent`.
