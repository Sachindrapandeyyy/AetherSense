---
name: aethersense-cli-api
description: Use the AetherSense `wifi-densepose` CLI binary (incl. MAT scan/status/zones/survivors/alerts/export subcommands), the REST API (`wifi-densepose-api`, Axum), and the browser/WASM build (`wifi-densepose-wasm`, `wifi-densepose-wasm-edge`). Use when integrating AetherSense into another program, scripting it from the shell, exposing it over HTTP, or shipping it to the browser / ESP32-WASM3.
allowed-tools: Bash Read Write Edit Glob Grep
---

# AetherSense CLI, API & WASM

The programmatic surfaces of AetherSense — the `wifi-densepose` binary, the HTTP API, and the WebAssembly builds.
...
| A long-running CSI sink + training | `wifi-densepose-sensing-server` (see `aethersense-applications` / `aethersense-model-training`) |
