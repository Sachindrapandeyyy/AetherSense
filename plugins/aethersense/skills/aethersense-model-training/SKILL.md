...
Make a model transfer across environments without retraining. Configured through the training pipeline's domain-generalization options; see ADR-027 and `wifi-densepose-train` + `aethersense_metrics`.
...
Then hand off to `aethersense-verify` for the witness bundle.
