...
- ADR-031 (AetherSense sensing-first RF mode), ADR-081 (adaptive CSI mesh firmware kernel), ADR-083 (per-cluster π compute hop), ADR-095/096 (on-ESP32 temporal modeling with sparse GQA attention — runs the temporal head on-device).
...
- Run a security review when touching anything on the hardware/network boundary (see `aethersense-verify` and `docs/security-audit-wasm-edge-vendor.md`).
