---
description: Run a AetherSense sensing application — presence, vitals, pose, sleep, environment mapping, MAT, point cloud, or a novel RF app.
argument-hint: "[presence|vitals|pose|sleep|environment|mat|pointcloud|<name>]"
---

# /aethersense-app

Launch a AetherSense application.

1. Invoke the **`aethersense-applications`** skill.
...
3. If no hardware: fall back to `docker run -p 3000:3000 ruvnet/wifi-densepose:latest` or `python examples/aethersense_live.py`.
4. Help pick the right modality (through-wall → presence/activity; stationary subject → vitals/sleep; need skeletons → pose, train it for accuracy; search & rescue → MAT; best accuracy → 2+ nodes + cross-viewpoint fusion via `/aethersense-advanced`).
