---
description: Provision WiFi credentials, sink IP, and optional channel / MAC-filter overrides onto a AetherSense ESP32 node.
argument-hint: "--port COM8 --ssid ... --password ... --target-ip ... [--channel N] [--filter-mac AA:BB:..]"
---

# /aethersense-provision

Write NVS config to an ESP32 sensing node.

1. Invoke the **`aethersense-configure`** skill (§"Runtime device config" — has the full `provision.py` flag table).
