---
description: Build and flash AetherSense ESP32 firmware (8MB or 4MB), then confirm the CSI stream.
argument-hint: "[8mb|4mb] [COM port]"
---

# /aethersense-flash

Build + flash AetherSense firmware to an ESP32-S3 sensing node.

1. Invoke the **`aethersense-hardware-setup`** skill.
...
6. If no frames: re-run `/aethersense-provision`, check channel matches the AP, drop any `--filter-mac`.
