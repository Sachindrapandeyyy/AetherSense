---
description: Get started with AetherSense — pick the fastest path (Docker demo, repo build, or live ESP32) and walk through it.
argument-hint: "[docker|build|hardware]"
---

# /aethersense-start

Onboard the user onto AetherSense (WiFi-DensePose).

1. Invoke the **`aethersense-quickstart`** skill.
2. If `$ARGUMENTS` names a tier (`docker`, `build`, `hardware`), go straight to it; otherwise ask which hardware they have:
   ...
   - **Have an ESP32-S3 / C6** → Tier 2: hand off to `/aethersense-flash` then `/aethersense-provision`, then `cargo run -p wifi-densepose-sensing-server`.
...
4. Point to next steps: `/aethersense-app`, `/aethersense-train`, `/aethersense-advanced`, `/aethersense-verify`, and the `aethersense-configure` skill.
