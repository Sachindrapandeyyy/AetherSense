...
- **D — domain generalization (MERIDIAN ADR-027):** domain-gen options in the training pipeline; `aethersense_metrics`.
...
Run the `aethersense-model-training` skill for canonical commands. Make the change, train, evaluate with the right metric (PCK@20 for pose), run the validation gate, then hand off to `/aethersense-verify`. Read before edit; no new files unless required; no secrets in commits.
