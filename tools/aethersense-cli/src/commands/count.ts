/**
 * aethersense count — Person count commands.
 *
  ...
  cli.command(
    ...
    (y) =>
      y
        ...
        .option("binary", {
          type: "string",
          description: "Path to cog-person-count binary (default: AETHERSENSE_COUNT_COG_BINARY)",
        })
        .option("max-persons", {
          type: "number",
          default: 7,
        ...
        if (!health.ok) {
          process.stderr.write(
            `[WARN] Cog health check failed: ${health.error}\n` +
              `Set AETHERSENSE_COUNT_COG_BINARY or install cog-person-count (ADR-103).\n`
          );
