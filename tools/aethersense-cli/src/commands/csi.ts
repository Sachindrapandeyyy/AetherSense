/**
 * aethersense csi — CSI frame commands.
 *
    ...
    (y) =>
      y
        ...
        .option("url", {
          type: "string",
          description:
            "Sensing-server URL (default: AETHERSENSE_SENSING_SERVER_URL or http://localhost:3000)",
        })
        .option("interval", {
          type: "number",
          default: 500,
    ...
    async (args) => {
      ...
      if (args.action === "tail") {
        process.stderr.write(
          `[aethersense csi tail] Streaming from ${baseUrl} every ${args.interval}ms. Ctrl-C to stop.\n`
        );
