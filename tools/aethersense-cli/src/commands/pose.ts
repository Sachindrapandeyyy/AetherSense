/**
 * aethersense pose — Pose estimation commands.
 *
  ...
  cli.command(
    ...
    (y) =>
      y
        ...
        .option("binary", {
          type: "string",
          description: "Path to cog-pose-estimation binary (default: AETHERSENSE_POSE_COG_BINARY)",
        }),
    async (args) => {
        ...
        if (!health.ok) {
          process.stderr.write(
            `[WARN] Cog health check failed: ${health.error}\n` +
              `Set AETHERSENSE_POSE_COG_BINARY or install cog-pose-estimation (ADR-101).\n`
          );
