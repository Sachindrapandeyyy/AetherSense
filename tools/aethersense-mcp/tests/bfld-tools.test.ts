 ...
 *    10. topic matches aethersense/<node_id>/bfld/* pattern
...
describe("aethersense.bfld.last_scan handler", () => {
...
describe("aethersense.bfld.last_scan schema (BfldLastScanInputSchema)", () => {
...
describe("aethersense.bfld.subscribe handler", () => {
  ...
  it("10. topic matches aethersense/<node_id>/bfld/* pattern", async () => {
    const r = await bfldSubscribe({ node_id: "seed-1", duration_s: 10 }, testConfig) as Record<string, unknown>;
    expect(r["topic"]).toBe("aethersense/seed-1/bfld/*");
  });
});

describe("aethersense.bfld.subscribe schema (BfldSubscribeInputSchema)", () => {
