/**
 * Configuration loader for the AetherSense MCP server.
 *
...
import type { AethersenseConfig } from "./types.js";
...
/**
 * Load the effective AethersenseConfig from environment variables.
 *
 * Environment variables:
 *   AETHERSENSE_SENSING_SERVER_URL   — base URL of the sensing-server  (default: http://localhost:3000)
 *   AETHERSENSE_API_TOKEN            — Bearer token for /api/v1/* routes (no default; auth disabled when absent)
 *   AETHERSENSE_POSE_COG_BINARY      — path to cog-pose-estimation binary
 *   AETHERSENSE_COUNT_COG_BINARY     — path to cog-person-count binary
 *   AETHERSENSE_JOBS_DIR             — directory for job logs (default: ~/.aethersense/jobs)
 */
export function loadConfig(): AethersenseConfig {
  return {
    sensingServerUrl: envOrDefault(
      "AETHERSENSE_SENSING_SERVER_URL",
      "http://localhost:3000"
    ),
    apiToken: env("AETHERSENSE_API_TOKEN"),
    poseCogBinary: envOrDefault(
      "AETHERSENSE_POSE_COG_BINARY",
      detectCogBinary("cog-pose-estimation")
    ),
    countCogBinary: envOrDefault(
      "AETHERSENSE_COUNT_COG_BINARY",
