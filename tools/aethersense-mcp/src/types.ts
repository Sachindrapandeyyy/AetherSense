/**
 * Shared domain types for the AetherSense MCP server.
 *
...
/** Output of aethersense_pose_infer. */
...
/** Output of aethersense_count_infer (ADR-103 person-count cog). */
...
/** Output of aethersense_registry_list. */
...
/** Output of aethersense_train_count — a job handle. */
export interface TrainJobResult {
  job_id: string;
  status: "queued" | "running" | "done" | "failed";
  /** Absolute path to the job log file (~/.aethersense/jobs/<id>.log). */
  log_path: string;
  /** Timestamp when the job was enqueued (seconds since epoch). */
  queued_at: number;
}

/** Output of aethersense_job_status. */
...
export interface AethersenseConfig {
  /** Base URL of the local sensing-server (default: http://localhost:3000). */
  sensingServerUrl: string;
  /** Bearer token for /api/v1/* endpoints. Set AETHERSENSE_API_TOKEN to enable. */
  ...
  /** Directory for job logs (default: ~/.aethersense/jobs/). */
  jobsDir: string;
}
