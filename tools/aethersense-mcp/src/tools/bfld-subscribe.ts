/**
 * MCP tool: aethersense.bfld.subscribe
 *
 ...
 * `aethersense/<node_id>/bfld/*` (ADR-122 §2.2) to a server-side event buffer.
 *
 ...
 * rather than throwing, consistent with the aethersense-mcp soft-failure convention.
 *
...
export async function bfldSubscribe(
  ...
  const topic = `aethersense/${nodeId}/bfld/*`;
