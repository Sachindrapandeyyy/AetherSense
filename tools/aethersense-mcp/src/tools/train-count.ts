/**
 * MCP tool: aethersense_train_count + aethersense_job_status
 *
 ...
 * returned job_id can be used with aethersense_job_status to poll progress.
 *
...
import type { AethersenseConfig, TrainJobResult, JobStatusResult } from "../types.js";
...
export const jobStatusSchema = z.object({
  job_id: z.string().uuid().describe("Job ID returned by aethersense_train_count."),
});

export type JobStatusInput = z.infer<typeof jobStatusSchema>;

// In-process job registry (survives for the lifetime of the MCP server process).
// For a production implementation, persist to ~/.aethersense/jobs/<id>.json.
...
export async function trainCount(
  ...
  // Record the job immediately so aethersense_job_status can find it.
  ...
  const header = [
    `# AetherSense training job ${jobId}`,
  ...
  return {
    ...
    note:
      ...
      `Poll progress with aethersense_job_status({ job_id: "${jobId}" }). ` +
      `Live log: ${logPath}`,
  };
}
