/**
 * MCP tool: aethersense_pose_infer
 *
...
import { z } from "zod";
import type { AethersenseConfig, PoseInferResult } from "../types.js";
import { runCog } from "../cog.js";

export const poseInferSchema = z.object({
  /**
   * Path to a CSI window JSON file (as produced by aethersense_csi_latest or
  ...
  cog_binary: z
    .string()
    .optional()
    .describe("Path to cog-pose-estimation binary. Default: AETHERSENSE_POSE_COG_BINARY env var."),
});
...
export async function poseInfer(
  ...
  if (!healthResult.ok) {
    return {
      ...
      hint:
        "Set AETHERSENSE_POSE_COG_BINARY to the path of the cog-pose-estimation binary. " +
  ...
  // this satisfies the ADR-104 acceptance gate: "aethersense_pose_infer returns a finite
