/**
 * MCP tool: aethersense_count_infer
 *
...
import { z } from "zod";
import type { AethersenseConfig, CountInferResult } from "../types.js";
...
export const countInferSchema = z.object({
  ...
  cog_binary: z
    .string()
    .optional()
    .describe("Path to cog-person-count binary. Default: AETHERSENSE_COUNT_COG_BINARY env var."),
  /**
...
export async function countInfer(
  ...
  if (!healthResult.ok) {
    return {
      ...
      hint:
        "Set AETHERSENSE_COUNT_COG_BINARY to the path of the cog-person-count binary. " +
