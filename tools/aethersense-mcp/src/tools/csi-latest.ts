/**
 * MCP tool: aethersense_csi_latest
 *
 ...
 * subcarrier data, feed it to aethersense_pose_infer, or store it for analysis.
 */

import { z } from "zod";
import type { AethersenseConfig, SensingLatestResponse } from "../types.js";
  ...
  if (!result.ok) {
    return {
      ...
      hint:
        ...
        "set AETHERSENSE_SENSING_SERVER_URL to the correct address.",
    };
  }
  ...
  if (!validation.valid) {
    return {
      ...
      hint:
        ...
        "aethersense-mcp/src/types.ts if needed.",
    };
  }
