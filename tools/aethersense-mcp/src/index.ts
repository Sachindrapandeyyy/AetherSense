#!/usr/bin/env node
/**
 * @ruv/aethersense-mcp — AetherSense MCP Server
 *
 * Exposes AetherSense's WiFi-DensePose sensing capabilities as Model Context Protocol
 * (MCP) tools that Claude Code, Cursor, Codex, and other MCP-compatible agents
 * can call directly.
 *
 * Tools exposed:
 *   aethersense_csi_latest    — pull the latest CSI window from the sensing-server
 *   aethersense_pose_infer    — single-shot 17-keypoint pose estimation
 *   aethersense_count_infer   — single-shot person count with confidence interval
 *   aethersense_registry_list — list cogs from the Cognitum edge registry (ADR-102)
 *   aethersense_train_count   — kick off a count-cog training run (returns job ID)
 *   aethersense_job_status    — poll a background training job
 *
 * Usage:
 *   node dist/index.js                   # stdio transport (default)
 *   AETHERSENSE_SENSING_SERVER_URL=http://cognitum-v0:3000 node dist/index.js
 *
 * To register with Claude Code:
 *   claude mcp add aethersense -- node /path/to/tools/aethersense-mcp/dist/index.js
 *
...
const TOOLS = [
    ...
    inputSchema: {
      ...
      properties: {
        sensing_server_url: {
          type: "string",
          description:
            "Base URL of the sensing-server (default: AETHERSENSE_SENSING_SERVER_URL or http://localhost:3000).",
        },
      },
    },
  ...
  {
    name: "aethersense_registry_list",
    description:
  ...
  },
  {
    ...
    description:
      ...
      "person-count labels. Returns a job_id to poll with aethersense_job_status.",
    inputSchema: {
      type: "object" as const,
  ...
  {
    name: "aethersense_job_status",
    description:
      "Poll the status of a background training job started by aethersense_train_count. " +
    ...
    inputSchema: {
      ...
      properties: {
        job_id: {
          type: "string",
          description: "UUID returned by aethersense_train_count.",
        },
      },
    },
    handler: async (args: unknown, config: ReturnType<typeof loadConfig>) => {
  ...
  {
    name: "aethersense.bfld.last_scan",
    description:
      ...
      "the MQTT state topics aethersense/<node_id>/bfld/* (ADR-122 §2.2).",
    inputSchema: {
      type: "object" as const,
      properties: {
        node_id: {
  ...
  {
    name: "aethersense.bfld.subscribe",
    description:
      "Subscribe to BFLD events on aethersense/<node_id>/bfld/* for duration_s seconds (ADR-122). " +
  ...
  {
    name: "aethersense.vitals.get_breathing",
    description:
  ...
  {
    name: "aethersense.vitals.get_heart_rate",
    description:
  ...
  {
    name: "aethersense.vitals.get_all",
    description:
...
main().catch((e) => {
  process.stderr.write(`[aethersense-mcp] Fatal: ${String(e)}\n`);
  process.exit(1);
});
