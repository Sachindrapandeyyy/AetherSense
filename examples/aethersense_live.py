#!/usr/bin/env python3
"""
AetherSense Live — Ambient Intelligence Dashboard with RuVector Signal Processing

Fuses WiFi CSI (ESP32-S3) + 60 GHz mmWave (MR60BHA2) with signal processing
algorithms ported from AetherSense's Rust crates:
...
Usage:
    python examples/aethersense_live.py --csi COM7 --mmwave COM4
"""
...
def run_display(hub, duration, interval, mode="vitals"):
    ...
    print("=" * 80)
    if mode == "happiness":
        print("  AetherSense Live — Happiness + Cognitum Seed Dashboard")
    else:
        print("  AetherSense Live — Ambient Intelligence + RuVector Signal Processing")
    print("=" * 80)
    print()
...
def main():
    parser = argparse.ArgumentParser(description="AetherSense Live + RuVector Analysis")
